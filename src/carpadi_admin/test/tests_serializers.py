...
class TransactionsSerializerTest(TestCase):
    ...
    def setUp(self):
        self.transactions_good_data = {'id': uuid.uuid4(),
            'wallet': uuid.uuid4(), 'amount': Decimal(5600.0000),
            'created': timezone.now()
        }

        self.transactions_invalid_id = {'id':"invalid id",
            'wallet': uuid.uuid4(), 'amount': Decimal(5600.0000),
            'created': timezone.now()
        }

        self.transactions_invalid_wallet_id = {'id': uuid.uuid4(),
            'wallet': "invalid wallet", 'amount': Decimal(5600.0000),
            'created': timezone.now()
        }

        self.transactions_invalid_amount = {'id': uuid.uuid4(),
            'wallet': uuid.uuid4(), 'amount': Decimal(5600.0000),
            'created': timezone.now()
        }
