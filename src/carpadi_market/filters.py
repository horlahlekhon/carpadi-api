...
class CarProductFilter(filters.FilterSet):
    ...
    fuel_type = filters.ChoiceFilter(
        field_name="car__fuel_type", lookup_expr="iexact", choices=FuelTypes.choices)
    car_type = filters.ChoiceFilter(
        field_name="car__car_type", lookup_expr="iexact", choices=CarTypes.choices)

    def search_field(self, queryset, name, value):
        resp = queryset.filter(
            Q(car__make__icontains=value) | Q(car__model__icontains=value)
            | Q(car__manufacturer__icontains=value))
        return resp
