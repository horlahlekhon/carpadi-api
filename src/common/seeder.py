...
from src.models.models import CarMerchant, User, UserTypes, Wallet, Car, CarBrand, CarMaintenance, \
    MiscellaneousExpenses, CarMaintenanceTypes, CarStates, Trade, TradeStates, TradeUnit
...
class PadiSeeder:
    ...
    def seed_merchants(self, count=1):
        self.seeder.add_entity(User, count, {
            "is_staff": False,
            "is_superuser": False,
            "is_active": True,
        ...
        wallet_ids = []
        for idx in user_ids:
            self.seeder.add_entity(CarMerchant, 1, {
    ...
    def seed_cars(self):
        ...
        self.seeder.add_entity(MiscellaneousExpenses, 1, {
        ...
        data = {"type": "expense", "cost": abs(self.seeder.faker.random_number(digits=4)),
    ...
    def seed_trade(self, car, status: TradeStates):
        assert status not in (
            TradeStates.Completed, TradeStates.Closed, TradeStates.Purchased), "Invalid status when creating trade"
        assert car.status == CarStates.Inspected, "Invalid car status when creating trade"
        self.seeder.add_entity(Trade, 1, {
            'car': car,
            'slots_available': 5,
