...
dummy = {'battery_kw': '', 'battery_type': '', 'battery_type_desc': '', 'battery_voltage_desc': '', 'body_style': 'UT',
       'body_style_desc': 'SPORT UTILITY VEHICLE', 'door_cnt': '4', 'drive_type': 'AWD',
       'drive_type_desc': 'All Wheel Drive', 'engine_has_supercharger': 'No', 'engine_has_turbocharger': 'No',
       'engine_has_variable_valve_timing': 'Yes', 'engine_block_type': 'V-type', 'engine_carbureted_barrel_cnt': 'NA',
       'engine_carbureted_type': 'Fuel Injection', 'engine_cylinder_cnt': '6', 'engine_displacement_cubic_cm': '',
       'engine_displacement_cubic_inches': '213', 'engine_displacement_cubic_liters': '3.5',
       'engine_fuel_injection': 'Multiport', 'engine_model': '', 'engine_manufacturer': '', 'tire_front_desc': '',
       'tire_front_pressure_lbs': '', 'tire_front_size': '17R245', 'make': 'HONDA', 'model': 'PILOT', 'year': str(random.randint(1900, 2023)),
       'mfg_base_msrp': '40670', 'manufacturer': 'Honda', 'origin_desc': 'Import Built in North America',
       'plant_city_name': 'LINCOLN', 'plant_country_name': 'United States', 'plant_location_name': 'ALABAMA',
       'tire_rear_desc': '', 'tire_rear_recommended_pressure': '', 'tire_rear_size': '17R245', 'weight_base': '4608',
       'trim': 'TOURING', 'trk_bed_length_code': 'R', 'trk_bed_length_desc': 'Regular', 'trk_brk_code': 'HYD',
       'trk_brk_desc': 'HYDRAULIC', 'trk_cab_code': 'SUV', 'trk_cab_desc': 'Sport Utility', 'wheel_end_cnt': '4',
       'wheel_powertrain_cnt': '4', 'fuel_cpcty_gallons': '21', 'vehicle_type': 'Truck', 'fuel_type': 'Gas',
...
class VinDecodeResponse(BaseModel):
    battery_kw: str = Field(default='',
                            description="The measure of total battery power expressed in kilowatts. For example: "
                                        "71KW, 85KW, 75KW, 67KW.")
    battery_type: str = Field(default='',
                              description="A value that identifies the kind of battery in the vehicle. For example: "
                                          "PbA - Lead Acid,NMH - Nickel Metal Hydride")
    battery_type_desc: str = Field(default='',
                                   description="The description of the code for the Battery Type Code. For "
                                               "example:PbA - Lead Acid,NMH - Nickel Metal Hydride.")
    battery_voltage_desc: str = Field(default='',
                                      description="The voltage rating of the battery as provided by the manufactuer.")
    body_style: str = Field(default='',
                            description="A code that describes the body style of the vehicle. For example, CP=Coupe")
    body_style_desc: str = Field(default='',
                                 description="The description of the code Body Style Code For example: Coupe ")
    door_cnt: int = Field(default=0, description="The number of doors the vehicle has.")
    drive_type: DriveTypes = Field(default=DriveTypes.Unknown,
                                   description="This element describes type of driving configuration for cars and "
                                               "trucks such as FWD, AWD, RWD.")
    engine_has_supercharger: EvilDichotomies = Field(default='',
                                                     description="Indicates if the engine has a supercharger or not. "
                                                                 "Yes, No or Unknown.")
    ...
    engine_carbureted_type: str = Field(default='',
    ...
    engine_cylinder_cnt: str = Field(default='',
                                     description="Contains a code that represents the number of cylinders a vehicle's "
                                                 "combustion engine can have.")
    engine_displacement_cubic_cm = Field(default='',
    ...
    engine_displacement_cubic_liters = Field(default='',
                                             description="(Displacement Liters) displacement in rounded Liters, "
                                                         ...
                                                         "displacement).")
    engine_fuel_injection = Field(default='',
                                  description="The type of fuel injection used by a vehicle. For example, Direct, "
                                              "Throttlebody")
    ...
    fuel_cpcty_gallons: str = Field(default='',
    ...
    fuel_type: str = Field(default='',
                           description="(Fuel) What an internal combustion burns to move a piston in a cylinder")
    tire_front_desc: str = Field(default='',
                                 description="more specific tire description (ex. Michelin Eagle P245/40ZR)")
    tire_front_pressure_lbs: str = Field(default='',
                                         description="(Front Tire Pressure) Vehicle Mfr reccomendation for tire "
                                                     "pressure, in pounds/sq. in.")
    ...
    year: int = Field(default=datetime.datetime.now().year,
                      description="The marketing year defined by the OEM which the vehicle was produced. The value "
                                  ...
                                  "year.")
    mfg_base_msrp: str = Field(default='',
    ...
    tire_rear_recommended_pressure: str = Field(default='',
                                                description="(Rear Tire Pressure) Vehicle Mfr reccomendation for tire "
                                                            "pressure, in pounds/sq. in.")
    tire_rear_size: str = Field(default='', description='(Rear Tire Size Description) As in "17R245"')
    weight_base: str = Field(default='',
    ...
    trk_bed_length_code: str = Field(default='',
    ...
    trk_brk_code: str = Field(default='',
    ...
    trk_cab_code: str = Field(default='',
                              description="(Cab Configuration) Cab Type describes the physical configuration of a "
                                          "truck's cabin.")
...
def check_vin(vin: str) -> Optional[VehicleInfoData]:
    if settings.APP_ENV != "prod":
        ...
        return  VehicleInfoData(
                engine=data.engine_model, transmission=data.transmission,
                car_type=data.body_style, fuel_type=data.fuel_type,
                age=datetime.datetime.now().year - data.year if data.year else 0,
                trim=data.trim, manufacturer=data.manufacturer, drive_type=data.drive_type.value,
                num_of_cylinders=data.engine_cylinder_cnt,
                raw_data=dummy, model=data.model, year=data.year
            )
    ...
    if response.status_code == 200:
        ...
        if meta.message == "ok":
            ...
            return VehicleInfoData(
                ...
                trim=data.trim, manufacturer=data.manufacturer, drive_type=data.drive_type.value,
                num_of_cylinders=data.engine_cylinder_cnt,
                raw_data=js.get("data"), model=data.model, year=data.year
            )
